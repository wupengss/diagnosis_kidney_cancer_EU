...
def bbox_transform_batch(ex_rois, gt_rois):
    ...
    if ex_rois.dim() == 2:
        ...
        targets_dx = (gt_ctr_x - ex_ctr_x.view(1,-1).expand_as(gt_ctr_x).cpu()) / ex_widths.cpu()
        targets_dy = (gt_ctr_y - ex_ctr_y.view(1,-1).expand_as(gt_ctr_y).cpu()) / ex_heights.cpu()
        targets_dw = torch.log(gt_widths / ex_widths.view(1,-1).expand_as(gt_widths).cpu())
        targets_dh = torch.log(gt_heights / ex_heights.view(1,-1).expand_as(gt_heights).cpu())
...
def bbox_transform_batch_3d(ex_rois, gt_rois):
    ...
    if ex_rois.dim() == 2:
        ...
        gt_ctr_x = gt_rois[:, :, 0] + 0.5 * gt_widths
        gt_ctr_y = gt_rois[:, :, 1] + 0.5 * gt_heights
        gt_ctr_z = gt_rois[:, :, 2] + 0.5 * gt_heights

        targets_dx = (gt_ctr_x - ex_ctr_x.view(1,-1).expand_as(gt_ctr_x).cpu()) / ex_widths.cpu()
        targets_dy = (gt_ctr_y - ex_ctr_y.view(1,-1).expand_as(gt_ctr_y).cpu()) / ex_heights.cpu()
        targets_dz = (gt_ctr_z - ex_ctr_z.view(1,-1).expand_as(gt_ctr_z).cpu()) / ex_slices.cpu()
        targets_dw = torch.log(gt_widths / ex_widths.view(1,-1).expand_as(gt_widths).cpu())
        targets_dh = torch.log(gt_heights / ex_heights.view(1,-1).expand_as(gt_heights).cpu())
    ...
    targets = torch.stack(
        (targets_dx, targets_dy, targets_dz, targets_dw, targets_dh, targets_ds),2)
...
def bbox_transform_inv_3d(boxes, deltas, batch_size):
    ...
    pred_ctr_z = dz * slices.unsqueeze(2) + ctr_z.unsqueeze(2)
    pred_w = torch.exp(dw) * widths.unsqueeze(2)
    pred_h = torch.exp(dh) * heights.unsqueeze(2)
    pred_s = torch.exp(ds) * slices.unsqueeze(2)
    ...
    # z2
    pred_boxes[:, :, 5::6] = pred_ctr_z + 0.5 * pred_s
...
def clip_boxes_batch_3d(boxes, im_shape, batch_size):
    """
    Clip boxes to image boundaries.
    """
    num_rois = boxes.size(1)
...
def clip_boxes_3d(boxes, im_shape, batch_size):
    ...
    for i in range(batch_size):
        boxes[i,:,0::6].clamp_(0, im_shape[i, 0, 1]-1)
        boxes[i,:,1::6].clamp_(0, im_shape[i, 0, 0]-1)
        boxes[i,:,2::6].clamp_(0, im_shape[i, 0, 2]-1)
        boxes[i,:,3::6].clamp_(0, im_shape[i, 0, 1]-1)
        boxes[i,:,4::6].clamp_(0, im_shape[i, 0, 0]-1)
        boxes[i,:,5::6].clamp_(0, im_shape[i, 0, 2]-1)
