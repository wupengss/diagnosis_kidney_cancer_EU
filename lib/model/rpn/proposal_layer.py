...
class _ProposalLayer3d(nn.Module):
    """
    Outputs object detection proposals by applying estimated bounding-box
    transformations to a set of regular boxes (called "anchors").
    """
    ...
    def __init__(self, feat_stride, scales):
        super(_ProposalLayer3d, self).__init__()

        self._feat_stride = feat_stride
        self._anchors = torch.from_numpy(generate_anchors_3d(scales=np.array(scales))).float()
        self._num_anchors = self._anchors.size(0)
    ...
    def forward(self, input):
        ...
        feat_height, feat_width, feat_slices = scores.size(2), scores.size(3), scores.size(4)
        shift_x = np.arange(0, feat_width) * self._feat_stride
        shift_y = np.arange(0, feat_height) * self._feat_stride
