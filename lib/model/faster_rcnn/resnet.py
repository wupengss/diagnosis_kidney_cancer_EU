...
class BasicBlock_3d(nn.Module):
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None):
    ...
    self.conv1 = nn.Conv3d(inplanes, planes, stride)
...
def resnet18_3d():
  ...
  Args:
    pretrained (bool): If True, returns a model pre-trained on ImageNet
  """
  model = ResNet_3d(BasicBlock_3d, [2, 2, 2, 2])
...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False, net):
    self.model_path = 'data/pretrained_model/resnet101_caffe.pth'
    self.dout_base_model = 1024
    self.pretrained = pretrained
    self.class_agnostic = class_agnostic
    if net == 'res18_3d':
      _fasterRCNN_3d.__init__(self, classes, class_agnostic)
    else:
